-- NovaLibrary v2.2 Potassium - Skonsolidowany Skrypt (Loadstring Version)
-- Wersja 2.2 - Czysty styl Potassium, lekko zaokrąglone przyciski, system ustawień GUI
--
-- Aby użyć, skopiuj i wklej do swojego executora:
-- local NovaLibrary = loadstring(game:HttpGet("https://raw.githubusercontent.com/DizzyUSER69/NovaLibrary/main/NovaLibrary_v2.2_Potassium.lua"))()
...
function Window:CreateGUI()
	...
	Globals.AddCornerRadius(self.MainFrame, 4)
...
function Window:CreateMinimizeButton()
	...
	Globals.AddCornerRadius(self.MinimizeButton, 4)
...
function Window:CreateCloseButton()
	...
	Globals.AddCornerRadius(self.CloseButton, 4)
...
function Window:CreateTab(tabName)
	...
	Globals.AddCornerRadius(tabButton, 4)
...
function Window:Minimize()
	...
	if self.IsMinimized then
		Globals.Animate(self.MainFrame, "Size", UDim2.new(0, self.Size.Width, 0, 35), 0.3)
		...
		Globals.Animate(self.MainFrame, "Size", UDim2.new(0, self.Size.Width, 0, self.Size.Height), 0.3)
...
function Button:CreateGUI()
	...
	self.MainFrame.BackgroundTransparency = ThemeManager:Get("Transparency")
	self.MainFrame.BorderSizePixel = 0
	self.MainFrame.Text = self.Text
	self.MainFrame.TextColor3 = ThemeManager:GetColor("TextPrimary")
	self.MainFrame.Font = ThemeManager:Get("Font")
	self.MainFrame.TextSize = ThemeManager:Get("FontSize")
	self.MainFrame.ZIndex = 1
	Globals.AddBorder(self.MainFrame, ThemeManager:GetColor("BorderColor"), ThemeManager:Get("BorderWidth"))
	Globals.AddCornerRadius(self.MainFrame, 4)
...
function Button:SetupEventHandlers()
	...
	Utilities.OnMouseEnter(self.MainFrame, function(hovering)
		if self.IsEnabled then 
			...
			self.MainFrame.BackgroundColor3 = hovering and ThemeManager:GetColor("Hover") or ThemeManager:GetColor("Foreground")
		end
	end)
end
...
function Toggle:CreateToggleButton()
	...
	Globals.AddCornerRadius(self.ToggleBackground, 4)
	...
	Globals.AddCornerRadius(self.ToggleButton, 4)
...
function Toggle:UpdateVisuals()
	if self.IsEnabled then
		Globals.Animate(self.ToggleBackground, "BackgroundColor3", ThemeManager:GetColor("Accent"), 0.1)
		Globals.Animate(self.ToggleButton, "BackgroundColor3", ThemeManager:GetColor("Background"), 0.1)
		Globals.Animate(self.ToggleButton, "Position", UDim2.new(0, 28, 0, 2), 0.2)
	else
		Globals.Animate(self.ToggleBackground, "BackgroundColor3", ThemeManager:GetColor("Foreground"), 0.1)
		Globals.Animate(self.ToggleButton, "BackgroundColor3", ThemeManager:GetColor("TextSecondary"), 0.1)
		Globals.Animate(self.ToggleButton, "Position", UDim2.new(0, 2, 0, 2), 0.2)
	end
end
...
-- ============================================
-- KOMPONENT SUWAKA (Slider Component)
-- ============================================

local Slider = {}
Slider.__index = Slider

function Slider:New(text, parent, min, max, default, callback)
...
function Slider:CreateGUI()
	...
	self.TextLabel.BackgroundTransparency = 1
	self.TextLabel.Text = self.Text .. ": " .. tostring(math.floor(self.Value))
	self.TextLabel.TextColor3 = ThemeManager:GetColor("TextPrimary")
	self.TextLabel.Font = ThemeManager:Get("Font")
	self.TextLabel.TextSize = ThemeManager:Get("FontSize")
	self.TextLabel.TextXAlignment = Enum.TextXAlignment.Left
...
-- ============================================
-- KOMPONENT LISTY ROZWIJANE (Dropdown Component)
-- ============================================

local Dropdown = {}
Dropdown.__index = Dropdown

function Dropdown:New(text, parent, options, callback)
...
function Dropdown:CreateGUI()
	...
	self.DropdownButton.Text = self.Text .. ": " .. self.Selected
	...
	Globals.AddCornerRadius(self.DropdownButton, 4)
	...
	Globals.AddCornerRadius(self.DropdownList, 4)
...
-- ============================================
-- KOMPONENT POLA TEKSTOWEGO (TextBox Component)
-- ============================================

local TextBox = {}
TextBox.__index = TextBox

function TextBox:New(text, parent, placeholder, callback)
...
function TextBox:CreateGUI()
	...
	Globals.AddCornerRadius(self.TextInputBox, 4)
...
-- ============================================
-- KOMPONENT POWIADOMIEŃ (Notification Component)
-- ============================================
...
function NovaLibrary:CreateSlider(text, parent, min, max, default, callback) return Slider:New(text, parent, min, max, default, callback) end
function NovaLibrary:CreateDropdown(text, parent, options, callback) return Dropdown:New(text, parent, options, callback) end
function NovaLibrary:CreateTextBox(text, parent, placeholder, callback) return TextBox:New(text, parent, placeholder, callback) end
function NovaLibrary:CreateNotification(title, message, duration) return Notification:New(title, message, duration) end
...
NovaLibrary:CreateButton("Example Button", tabCombat.Content, function()
	NovaLibrary:CreateNotification("Combat", "Button pressed!", 2)
end)
NovaLibrary:CreateToggle("Example Toggle", tabCombat.Content, function(state)
	NovaLibrary:Log("Toggle: " .. tostring(state), "INFO")
end)
NovaLibrary:CreateSlider("Example Slider", tabCombat.Content, 0, 100, 50, function(value)
	NovaLibrary:Log("Value: " .. tostring(math.floor(value)), "INFO")
end)

-- KATEGORIA: VISUALS
local tabVisuals = mainWindow:CreateTab("Visuals")
NovaLibrary:CreateButton("Example Button", tabVisuals.Content, function()
	NovaLibrary:CreateNotification("Visuals", "Button pressed!", 2)
end)
NovaLibrary:CreateToggle("Example Toggle", tabVisuals.Content, function(state)
	NovaLibrary:Log("Toggle: " .. tostring(state), "INFO")
end)
NovaLibrary:CreateDropdown("Example Dropdown", tabVisuals.Content, {"Option 1", "Option 2", "Option 3"}, function(selected)
	NovaLibrary:Log("Selected: " .. selected, "INFO")
end)

-- KATEGORIA: MISC
local tabMisc = mainWindow:CreateTab("Misc")
NovaLibrary:CreateButton("Example Button", tabMisc.Content, function()
	NovaLibrary:CreateNotification("Misc", "Button pressed!", 2)
end)
NovaLibrary:CreateTextBox("Example TextBox", tabMisc.Content, "Enter text...", function(text)
	NovaLibrary:Log("Text: " .. text, "INFO")
end)
NovaLibrary:CreateToggle("Example Toggle", tabMisc.Content, function(state)
	NovaLibrary:Log("Toggle: " .. tostring(state), "INFO")
end)

-- KATEGORIA: SETTINGS
local tabSettings = mainWindow:CreateTab("Settings")
NovaLibrary:CreateButton("Save Settings", tabSettings.Content, function()
	NovaLibrary:CreateNotification("Settings", "Settings saved!", 2)
end)
NovaLibrary:CreateToggle("Dark Mode", tabSettings.Content, function(state)
	if state then 
		NovaLibrary:LoadTheme("Dark")
	else 
		NovaLibrary:LoadTheme("Potassium") 
	end
end)
NovaLibrary:CreateSlider("GUI Opacity", tabSettings.Content, 0, 1, 0.95, function(value)
	NovaLibrary:Log("Opacity: " .. tostring(value), "INFO")
end)
NovaLibrary:CreateDropdown("GUI Theme", tabSettings.Content, {"Potassium", "Dark"}, function(selected)
	NovaLibrary:LoadTheme(selected)
end)

mainWindow:OnWindowClose(function()
	NovaLibrary:Log("Script closed", "INFO")
end)

mainWindow:OnWindowMinimize(function(isMinimized)
	if isMinimized then
		NovaLibrary:Log("Script minimized", "INFO")
	else
		NovaLibrary:Log("Script restored", "INFO")
	end
end)

NovaLibrary:Log("NovaLibrary v2.2 Potassium loaded successfully!", "INFO")
print("NovaLibrary v2.2 Potassium loaded!")
